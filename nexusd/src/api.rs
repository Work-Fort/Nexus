            ...
            match state.vsock_manager.connect_and_handshake(vm.id, runtime_dir.clone()).await {
                Ok(metadata) => {
                    ...
                    if let Err(e) = state.vsock_manager.connect_mcp(vm.id, runtime_dir.clone()).await {
                        ...
                        // Configure DNS via MCP file_write
                        if let Ok(Some(network_cfg)) = state.store.get_vm_network(vm.id.as_i64()) {
                            let dns_servers = state.network_service.dns_servers();
                            let resolv_conf = format!(
                                "# Generated by nexusd\n{}\n",
                                dns_servers
                                    .split(',')
                                    .map(|s| format!("nameserver {}", s.trim()))
                                    .collect::<Vec<_>>()
                                    .join("\n")
                            );

                            match state.vsock_manager.get_mcp_connection(vm.id, runtime_dir.clone()).await {
                                Ok(stream) => {
                                    let mcp_client = nexus_lib::mcp_client::McpClient::new(stream);
                                    match mcp_client.file_write("/etc/resolv.conf", &resolv_conf).await {
                                        Ok(_) => {
                                            tracing::info!("Configured DNS for VM {} at {}", vm.name, network_cfg.ip_address);
                                        }
                                        Err(e) => {
                                            tracing::error!("Failed to write DNS config for VM {}: {}", vm.name, e);
                                        }
                                    }
                                }
                                Err(e) => {
                                    tracing::warn!("Failed to get MCP connection for DNS config: {}", e);
                                }
                            }
                        }
                    }
